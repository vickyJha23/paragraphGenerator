...
//empty string ko parseInt NaN me convert kar deta hain
form.addEventListener("submit", (e) => {
      e.preventDefault();
      const value = parseInt(amount.value);
      const random = Math.floor(Math.random() * text.length);
      //isNaN is function in javascript which check the given value if it is then it return true, or false.... 
      if(isNaN(value) || value < 0 || value > 9){
          result.innerHTML = `<p class="result">${text[random]}`;
      }
      else{
        //slice method takes two parameter... slice(start, end)//end is not included if one parameter is given it slice off the rest of the thing....
         let tempText = text.slice(0, value);
         console.log("before map: ", tempText);
